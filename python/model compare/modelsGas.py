...
def plot_anomalies(model, test_data, title, ax):
    ...
    test_predictions = model.predict(test_data[['Oxygen']])
    ...
    ax.plot(test_data['id'], test_data['Oxygen'], label='Datos Normales', color='blue', alpha=0.7)
    ax.scatter(detected_anomalies['id'], detected_anomalies['Oxygen'], label='Anomalías Detectadas', color='green', marker='o')
    ax.scatter(undetected_anomalies['id'], undetected_anomalies['Oxygen'], label='Anomalías No Detectadas', color='red', marker='o')
...
df = pd.read_csv("model compare\\Gas.csv", header=None, names=["id", "Hour", "Nitrogen" , "Oxygen" , "NitrousGas", 
                                                "CarbonMonoxide", "CarbonAnidide", "SulphurousAnidide", 
                                                "FiredampGas", "Hydrosulphur"], parse_dates=[1], 
                                                infer_datetime_format=True)
df_anomalia = pd.read_csv('model compare\\GasAnomalies.csv', header=None, names=['id', 'anomalia'])
...
for model in models:
    # Entrenar el modelo
    model.fit(train_data[["Nitrogen" , "Oxygen" , "NitrousGas", 
                                                "CarbonMonoxide", "CarbonAnidide", "SulphurousAnidide", 
                                                "FiredampGas", "Hydrosulphur"]], train_data['anomalia'])

    # Predecir en los datos de prueba
    test_predictions = model.predict(test_data[["Nitrogen" , "Oxygen" , "NitrousGas", 
                                                "CarbonMonoxide", "CarbonAnidide", "SulphurousAnidide", 
...
for model, ax in zip(models, axes.flatten()):
    model.fit(train_data[['Oxygen']], train_data['anomalia'])
